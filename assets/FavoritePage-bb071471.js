import{n as t,P as d,j as e,h as l,a as y,b as J,c as j,r as c,t as k,M as z,v as x,w as T}from"./index-fed2c0c3.js";import{M as K}from"./MainPageTitle-bff4df4f.js";import{s as h,a as v}from"./favorite-selector-94e92a4d.js";import{S as F,D as L}from"./DeleteButton-7f6c14fc.js";import{S as M,M as Z,a as B}from"./RecipesList.styled-03ffb904.js";import{P as H}from"./Pagination-2983fa83.js";import{E as R}from"./ErrorPage.styled-452f6e48.js";import"./assertThisInitialized-85b66fac.js";const G=t.div`
  display: flex;
  flex-direction: column;
  border: none;
  gap: 24px;

  @media screen and (min-width: 768px) {
    justify-content: space-between;
    max-width: 342px;
  }

  @media screen and (min-width: 1440px) {
    max-width: 400px;
  }
`,O=t.img`
  display: block;
  transition: transform 600ms ease, box-shadow 400ms ease;
  background: linear-gradient(
    180deg,
    rgba(10, 10, 17, 0.024) 51.18%,
    rgba(10, 10, 17, 0.768) 97.66%
  );

  border-radius: 8px;
  box-shadow: 5px 4px 8px 0px rgba(34, 60, 80, 0.2);
  &:hover,
  &:focus {
    transform: scale(0.98);
    box-shadow: 0 0 12px rgba(243, 243, 243, 0.5);
  }
  @media screen and (min-width: 768px) {
  }

  @media screen and (min-width: 1440px) {
    height: 400px;
  }
`,X=t.div`
  /* display: flex;
  flex-direction: column;
 
  justify-content: space-between; */
  /* height: 124px; */
  gap: 18px;
  @media screen and (min-width: 768px) {
    /* display: flex; */
    /* flex-direction: column;
    justify-content: space-between; */
    gap: 24px;
    /* height: 252px; */
  }

  @media screen and (min-width: 1440px) {
    /* display: flex;
    flex-direction: column;
    justify-content: space-between; */
    /* height: 252px; */
  }
`,P=t.h3`
  /* margin-bottom: 4px; */
  font-family: Manrope;
  font-size: 18px;
  font-weight: 500;
  line-height: 24px;
  letter-spacing: 0em;
  text-align: left;

  color: ${i=>i.theme.colors.white};

  @media screen and (min-width: 768px) {
    font-family: Manrope;
    font-size: 24px;

    line-height: 32px;
  }
`,S=t.h3`
  font-family: Manrope;
  font-size: 18px;
  font-weight: 500;
  line-height: 24px;
  letter-spacing: 0em;
  text-align: left;

  color: ${i=>i.theme.colors.white};

  @media screen and (min-width: 768px) {
    font-family: Manrope;
    font-size: 16px;
    font-weight: 500;
    line-height: 18px;
    letter-spacing: 0em;
    text-align: left;
  }
`,N=t.p`
  display: inline-block;
  height: 45px;
  font-family: Manrope;
  font-size: 14px;
  font-weight: 400;
  line-height: 18px;
  letter-spacing: 0em;
  text-align: left;
  overflow: hidden;
  color: ${i=>i.theme.colors.white};

  @media screen and (min-width: 768px) {
    display: inline-block;
    font-family: Manrope;
    font-size: 16px;
    font-weight: 400;
    line-height: 22px;
    letter-spacing: 0em;
    text-align: left;
  }

  @media screen and (min-width: 1440px) {
    display: inline-block;

    font-family: Manrope;
    font-size: 16px;
    font-weight: 400;
    line-height: 22px;
    letter-spacing: 0em;
    text-align: left;
  }
`,Y=t.p`
  display: flex;
  gap: 18px;

  @media screen and (min-width: 768px) {
  }

  @media screen and (min-width: 1440px) {
  }
`,E="data:image/png;base64,",f=({recipe:i,removeCocktail:n})=>e.jsxs(G,{children:[e.jsx(O,{src:i.drinkThumb?i.drinkThumb:E,alt:i.drink}),e.jsxs(X,{children:[e.jsx(P,{children:i.drink}),e.jsx(S,{children:i.glass})]}),e.jsx(N,{children:i.description||"Good cocktail"}),e.jsxs(Y,{children:[e.jsx(F,{id:i._id}),e.jsx(L,{id:i._id,text:"Delete",ariaLabel:"button for click",onClick:()=>n(i._id),type:"button"})]})]});f.propTypes={recipe:d.oneOfType([d.object,d.array])};const V=({currentItems:i,handleDeleteFavorite:n})=>{const o=l(h);return e.jsx(M,{children:o&&e.jsx(y,{})||e.jsx(Z,{children:i.map(s=>e.jsx("li",{children:e.jsx(f,{recipe:s,page:"favorite",cocktail:s,removeCocktail:n})},s._id))})})};t.section`
  padding-top: 80px;
  /* padding-bottom: 80px; */
  @media (min-width: 768px) {
    padding-top: 140px;
    /* padding-bottom: 136px; */
  }
  @media (min-width: 1440px) {
    flex-direction: row;
    gap: 94px;
    padding-top: 160px;
    /* padding-bottom: 140px; */
  }
`;const ie=()=>{J();const i=j(),n=l(v);l(h);const[o,s]=c.useState(()=>window.innerWidth>=1440?9:8);c.useEffect(()=>{const a=()=>{window.innerWidth>=1440?s(9):s(8)};return window.addEventListener("resize",a),()=>window.removeEventListener("resize",a)},[]);const[A,w]=c.useState(1),p=A*o,C=p-o,g=n.slice(C,p),[m,I]=c.useState(1);c.useEffect(()=>{i(k()),I(Math.ceil(n.length/o))},[]);const u=a=>{x.success("Deleting..."),i(T(a)).unwrap().then(r=>{I(Math.ceil(r.totalHits/o))}).then(()=>x.success("Delete success!"))},b=(a,r)=>{w(r)};return e.jsx(e.Fragment,{children:e.jsx(z,{children:e.jsxs(e.Fragment,{children:[e.jsx(K,{title:"Favorites"}),(g==null?void 0:g.length)!==0?e.jsx(V,{handleDeleteFavorite:u,currentItems:g}):e.jsxs(e.Fragment,{children:[e.jsx(R,{}),e.jsx(B,{children:"You havent added any favorite cocktails yet"})]}),m>1&&e.jsx(H,{totalPages:m,page:A,changeNum:b})]})})})};export{ie as default};
