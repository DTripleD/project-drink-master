import{n as s,P as l,j as e,h as A,a as y,r as o,b as J,c as j,v as k,M as T,w as f,x as z}from"./index-0ed355f5.js";import{M as K}from"./MainPageTitle-5bcc18b2.js";import{s as h,a as F}from"./favorite-selector-94e92a4d.js";import{S as L,D as M}from"./DeleteButton-b7ccea24.js";import{S as Z,M as v,a as B}from"./RecipesList.styled-ec279717.js";import{P as H}from"./Pagination-ed5cc146.js";import{E as G}from"./ErrorPage.styled-12c5338c.js";import"./assertThisInitialized-d52680e6.js";import"./white-bg@2x-60be6de5.js";const R=s.div`
  display: flex;
  flex-direction: column;
  border: none;
  gap: 24px;

  @media screen and (min-width: 768px) {
    justify-content: space-between;
    max-width: 342px;
  }

  @media screen and (min-width: 1440px) {
    max-width: 400px;
  }
`,O=s.img`
  display: block;
  transition: transform 600ms ease, box-shadow 400ms ease;
  background: linear-gradient(
    180deg,
    rgba(10, 10, 17, 0.024) 51.18%,
    rgba(10, 10, 17, 0.768) 97.66%
  );

  border-radius: 8px;
  box-shadow: 5px 4px 8px 0px rgba(34, 60, 80, 0.2);
  &:hover,
  &:focus {
    transform: scale(0.98);
    box-shadow: 0 0 12px rgba(243, 243, 243, 0.5);
  }
  @media screen and (min-width: 768px) {
  }

  @media screen and (min-width: 1440px) {
    height: 400px;
  }
`,X=s.div`
  /* display: flex;
  flex-direction: column;
 
  justify-content: space-between; */
  /* height: 124px; */
  gap: 18px;
  @media screen and (min-width: 768px) {
    /* display: flex; */
    /* flex-direction: column;
    justify-content: space-between; */
    gap: 24px;
    /* height: 252px; */
  }

  @media screen and (min-width: 1440px) {
    /* display: flex;
    flex-direction: column;
    justify-content: space-between; */
    /* height: 252px; */
  }
`,P=s.h3`
  /* margin-bottom: 4px; */
  font-family: Manrope;
  font-size: 18px;
  font-weight: 500;
  line-height: 24px;
  letter-spacing: 0em;
  text-align: left;

  color: ${t=>t.theme.colors.white};

  @media screen and (min-width: 768px) {
    font-family: Manrope;
    font-size: 24px;

    line-height: 32px;
  }
`,S=s.h3`
  font-family: Manrope;
  font-size: 18px;
  font-weight: 500;
  line-height: 24px;
  letter-spacing: 0em;
  text-align: left;

  color: ${t=>t.theme.colors.white};

  @media screen and (min-width: 768px) {
    font-family: Manrope;
    font-size: 16px;
    font-weight: 500;
    line-height: 18px;
    letter-spacing: 0em;
    text-align: left;
  }
`,N=s.p`
  display: inline-block;
  height: 45px;
  font-family: Manrope;
  font-size: 14px;
  font-weight: 400;
  line-height: 18px;
  letter-spacing: 0em;
  text-align: left;
  overflow: hidden;
  color: ${t=>t.theme.colors.white};

  @media screen and (min-width: 768px) {
    display: inline-block;
    font-family: Manrope;
    font-size: 16px;
    font-weight: 400;
    line-height: 22px;
    letter-spacing: 0em;
    text-align: left;
  }

  @media screen and (min-width: 1440px) {
    display: inline-block;

    font-family: Manrope;
    font-size: 16px;
    font-weight: 400;
    line-height: 22px;
    letter-spacing: 0em;
    text-align: left;
  }
`,Y=s.p`
  display: flex;
  gap: 18px;

  @media screen and (min-width: 768px) {
  }

  @media screen and (min-width: 1440px) {
  }
`,E="data:image/png;base64,",w=({recipe:t,removeCocktail:i})=>e.jsxs(R,{children:[e.jsx(O,{src:t.drinkThumb?t.drinkThumb:E,alt:t.drink}),e.jsxs(X,{children:[e.jsx(P,{children:t.drink}),e.jsx(S,{children:t.glass})]}),e.jsx(N,{children:t.description||"Good cocktail"}),e.jsxs(Y,{children:[e.jsx(L,{id:t._id}),e.jsx(M,{id:t._id,text:"Delete",ariaLabel:"button for click",onClick:()=>i(t._id),type:"button"})]})]});w.propTypes={recipe:l.oneOfType([l.object,l.array])};const V=({currentItems:t,handleDeleteFavorite:i})=>{const a=A(h);return e.jsx(Z,{children:a&&e.jsx(y,{})||e.jsx(v,{children:t.map(c=>e.jsx("li",{children:e.jsx(w,{recipe:c,page:"favorite",cocktail:c,removeCocktail:i})},c._id))})})};s.section`
  padding-top: 80px;
  /* padding-bottom: 80px; */
  @media (min-width: 768px) {
    padding-top: 140px;
    /* padding-bottom: 136px; */
  }
  @media (min-width: 1440px) {
    flex-direction: row;
    gap: 94px;
    padding-top: 160px;
    /* padding-bottom: 140px; */
  }
`;const ie=()=>{o.useEffect(()=>{window.scrollTo(0,0)},[]),J();const t=j(),i=A(F);A(h);const[a,c]=o.useState(()=>window.innerWidth>=1440?9:8);o.useEffect(()=>{const n=()=>{window.innerWidth>=1440?c(9):c(8)};return window.addEventListener("resize",n),()=>window.removeEventListener("resize",n)},[]);const[r,p]=o.useState(1),m=r*a,C=m-a,I=i.slice(C,m),[d,x]=o.useState(1);o.useEffect(()=>{t(k()),x(Math.ceil(i.length/a))},[t]),o.useEffect(()=>{I.length===0&&r!==1&&p(n=>n-1)},[d,r]);const u=n=>{f.success("Deleting..."),t(z(n)).unwrap().then(g=>{x(Math.ceil(g.totalHits/a))}).then(()=>f.success("Delete success!"))},b=(n,g)=>{p(g)};return e.jsx(e.Fragment,{children:e.jsx(T,{children:e.jsxs(e.Fragment,{children:[e.jsx(K,{title:"Favorites"}),(i==null?void 0:i.length)!==0?e.jsx(V,{handleDeleteFavorite:u,currentItems:I}):e.jsxs(e.Fragment,{children:[e.jsx(G,{}),e.jsx(B,{children:"You havent added any favorite cocktails yet"})]}),d>1&&e.jsx(H,{totalPages:d,page:r,changeNum:b})]})})})};export{ie as default};
