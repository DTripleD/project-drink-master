import{n as t,P as d,j as e,h as l,a as u,b as y,c as J,r as c,t as j,M as k,v as z}from"./index-865b7153.js";import{M as T}from"./MainPageTitle-00fb8c35.js";import{s as x,a as K}from"./favorite-selector-94e92a4d.js";import{S as v,D as F}from"./DeleteButton-86a592ae.js";import{S as L,M,a as Z}from"./RecipesList.styled-e36d083a.js";import{P as B}from"./Pagination-d57c6142.js";import{E as H}from"./ErrorPage.styled-cdae8f3a.js";import"./assertThisInitialized-2a3f4a21.js";const R=t.div`
  display: flex;
  flex-direction: column;
  border: none;
  gap: 24px;

  @media screen and (min-width: 768px) {
    justify-content: space-between;
    max-width: 342px;
  }

  @media screen and (min-width: 1440px) {
    max-width: 400px;
  }
`,G=t.img`
  display: block;
  transition: transform 600ms ease, box-shadow 400ms ease;
  background: linear-gradient(
    180deg,
    rgba(10, 10, 17, 0.024) 51.18%,
    rgba(10, 10, 17, 0.768) 97.66%
  );

  border-radius: 8px;
  box-shadow: 5px 4px 8px 0px rgba(34, 60, 80, 0.2);
  &:hover,
  &:focus {
    transform: scale(0.98);
    box-shadow: 0 0 12px rgba(243, 243, 243, 0.5);
  }
  @media screen and (min-width: 768px) {
  }

  @media screen and (min-width: 1440px) {
    height: 400px;
  }
`,O=t.div`
  /* display: flex;
  flex-direction: column;
 
  justify-content: space-between; */
  /* height: 124px; */
  gap: 18px;
  @media screen and (min-width: 768px) {
    /* display: flex; */
    /* flex-direction: column;
    justify-content: space-between; */
    gap: 24px;
    /* height: 252px; */
  }

  @media screen and (min-width: 1440px) {
    /* display: flex;
    flex-direction: column;
    justify-content: space-between; */
    /* height: 252px; */
  }
`,X=t.h3`
  /* margin-bottom: 4px; */
  font-family: Manrope;
  font-size: 18px;
  font-weight: 500;
  line-height: 24px;
  letter-spacing: 0em;
  text-align: left;

  color: ${i=>i.theme.colors.white};

  @media screen and (min-width: 768px) {
    font-family: Manrope;
    font-size: 24px;

    line-height: 32px;
  }
`,P=t.h3`
  font-family: Manrope;
  font-size: 18px;
  font-weight: 500;
  line-height: 24px;
  letter-spacing: 0em;
  text-align: left;

  color: ${i=>i.theme.colors.white};

  @media screen and (min-width: 768px) {
    font-family: Manrope;
    font-size: 16px;
    font-weight: 500;
    line-height: 18px;
    letter-spacing: 0em;
    text-align: left;
  }
`,S=t.p`
  display: inline-block;
  height: 45px;
  font-family: Manrope;
  font-size: 14px;
  font-weight: 400;
  line-height: 18px;
  letter-spacing: 0em;
  text-align: left;
  overflow: hidden;
  color: ${i=>i.theme.colors.white};

  @media screen and (min-width: 768px) {
    display: inline-block;
    font-family: Manrope;
    font-size: 16px;
    font-weight: 400;
    line-height: 22px;
    letter-spacing: 0em;
    text-align: left;
  }

  @media screen and (min-width: 1440px) {
    display: inline-block;

    font-family: Manrope;
    font-size: 16px;
    font-weight: 400;
    line-height: 22px;
    letter-spacing: 0em;
    text-align: left;
  }
`,N=t.p`
  display: flex;
  gap: 18px;

  @media screen and (min-width: 768px) {
  }

  @media screen and (min-width: 1440px) {
  }
`,Y="data:image/png;base64,",f=({recipe:i,removeCocktail:n})=>e.jsxs(R,{children:[e.jsx(G,{src:i.drinkThumb?i.drinkThumb:Y,alt:i.drink}),e.jsxs(O,{children:[e.jsx(X,{children:i.drink}),e.jsx(P,{children:i.glass})]}),e.jsx(S,{children:i.description||"Good cocktail"}),e.jsxs(N,{children:[e.jsx(v,{id:i._id}),e.jsx(F,{id:i._id,text:"Delete",ariaLabel:"button for click",onClick:()=>n(i._id),type:"button"})]})]});f.propTypes={recipe:d.oneOfType([d.object,d.array])};const E=({currentItems:i,handleDeleteFavorite:n})=>{const o=l(x);return e.jsx(L,{children:o&&e.jsx(u,{})||e.jsx(M,{children:i.map(a=>e.jsx("li",{children:e.jsx(f,{recipe:a,page:"favorite",cocktail:a,removeCocktail:n})},a._id))})})};t.section`
  padding-top: 80px;
  /* padding-bottom: 80px; */
  @media (min-width: 768px) {
    padding-top: 140px;
    /* padding-bottom: 136px; */
  }
  @media (min-width: 1440px) {
    flex-direction: row;
    gap: 94px;
    padding-top: 160px;
    /* padding-bottom: 140px; */
  }
`;const ee=()=>{y();const i=J(),n=l(K);l(x);const[o,a]=c.useState(()=>window.innerWidth>=1440?9:8);c.useEffect(()=>{const s=()=>{window.innerWidth>=1440?a(9):a(8)};return window.addEventListener("resize",s),()=>window.removeEventListener("resize",s)},[]);const[A,h]=c.useState(1),p=A*o,w=p-o,g=n.slice(w,p),[m,I]=c.useState(1);c.useEffect(()=>{i(j()),I(Math.ceil(n.length/o))},[]);const C=s=>{i(z(s)).unwrap().then(r=>{I(Math.ceil(r.totalHits/o))})},b=(s,r)=>{h(r)};return e.jsx(e.Fragment,{children:e.jsx(k,{children:e.jsxs(e.Fragment,{children:[e.jsx(T,{title:"Favorites"}),(g==null?void 0:g.length)!==0?e.jsx(E,{handleDeleteFavorite:C,currentItems:g}):e.jsxs(e.Fragment,{children:[e.jsx(H,{}),e.jsx(Z,{children:"You havent added any favorite cocktails yet"})]}),m>1&&e.jsx(B,{totalPages:m,page:A,changeNum:b})]})})})};export{ee as default};
